function anatomyExtraction(pathData, pathBW, pathWholeBW, saveDataPath, saveBWPath)   
    warning('off','all');
    warning;
    close all;
     cd(pathBW);
    ...
    for ii=1:nfiles
       ...
       % read head + back image
       currentfilename = imagefiles(ii).name;
       BWImage = imread(currentfilename);
       ...
       for regionAll = 1 : length(stats)
              ...
              [ bwSubImageRotated, subImageRotated] = eliminateBoundaryData( regionAll, stats, BwWhole, BW, dataImage, BWImage);
...
function [ bwSubImageRotated, subImageRotated] = eliminateBoundaryData( regionAll, stats, BwWhole, BW, dataImage, BWImage)
...
anyX0 = any(X == 1);anyY0 = any(Y == 1);anyXw = any(X == size(BW,2));anyYh = any(Y == size(BW,1));

% elminate boundary data from 3 images
if (stats(regionAll).Area < 70000 || stats(regionAll).Area > 150000 || anyX0 ~= 0 || anyY0 ~= 0|| anyXw ~= 0|| anyYh ~= 0)  
    ...
    for x = 1:  size(X,1)
     ...
     [bwSubImageRotated, subImageRotated] = cropRotate(regionAll, stats, BWImage, dataImage, BwWhole);                

end 
end

function [bwSubImageRotated, subImageRotated] = cropRotate(regionAll, stats, BWImage, dataImage, BwWhole)
...
[bwSubImageRotated, subImageRotated] = keepLargestBlob(bwSubImage, subImage, statsWhole);
